...
const register = catchAsync(async (req, res) => {
  ...
  res.status(httpStatus.CREATED).send({
    ...
    message:
      'Valid information. Move to confirm email before sending email activation.',
  });
});

const sendVerificationEmail = catchAsync(async (req, res) => {
  const { email } = req.body;
  const token = await tokenService.generateTokenForSendVerificationEmail(
    req.body
  );
...
const activateEmail = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const { firstName, lastName, fullName, email, password, birthday, sex } =
      jwt.decode(token);
        ...
        User.findOne({ email }).exec((err, user) => {
          if (user) {
            ...
            return res
              ...
              .send({ user: temp, accessToken, refreshToken });
          } else {
...
const checkLogin = catchAsync(async (req, res) => {
  ...
  const token = tokenService.checkAccessTokenAndRefreshToken(
    accessToken,
    refreshToken
  );
